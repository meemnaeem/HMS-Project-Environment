<?php

namespace App\Http\Livewire\Admin\Appointment;

use App\Models\Appointment;
use Livewire\Component;
use Illuminate\Support\Facades\Route;
use Illuminate\Support\Facades\Validator;

class Appointments extends Component
{
    public $state = [];
    public $showEditModal = false;
    public $appointment;
    public $appointmentIdBeingRemoved = null;
    public $selectAll = false;
    public $checked = [];
    public $search = '';
    public $currentUrl;
    public $deleteId = '';
    public $appointmentsQuery = '';


    public function mount()
    {
        $this->currentUrl = Route::current()->getName();
    }

    public function getAppointmentsQueryProperty()
    {
        return Appointment::search($this->search);
    }

    public function addNew()
    {
        $this->state = [];
        $this->showEditModal = false;
        $this->dispatchBrowserEvent('show-modal');
    }

    public function validateData()
    {
        $validData = Validator::make($this->state, [
            'patient_id' => 'required',
            'user_id' => 'required',
            'date' => 'required',
        ])->validate();
        return $validData;
    }

    public function createAppointment()
    {
        $validatedData = $this->validateData();
        Appointment::create($validatedData);
        $this->dispatchBrowserEvent('hide-modal', ['message' => 'Appointment added successfully!']);
    }

    public function edit(Appointment $appointment)
    {
        $this->showEditModal = true;
        $this->appointment = $appointment;
        $this->state = $appointment->toArray();
        $this->dispatchBrowserEvent('show-modal');
    }

    public function updateAppointment()
    {
        $validatedData = $this->validateData();
        $this->appointment->update($validatedData);
        $this->dispatchBrowserEvent('hide-modal', ['message' => 'Appointment updated successfully!']);
    }

    public function confirmAppointmentRemoval($appointmentId)
    {
        $this->appointmentIdBeingRemoved = $appointmentId;
        $this->dispatchBrowserEvent('show-delete-modal');
    }

    public function deleteAppointment()
    {
        $appointment = Appointment::findOrFail($this->appointmentIdBeingRemoved);
        $appointment->delete();
        $this->dispatchBrowserEvent('hide-delete-modal', ['message' => 'Appointment deleted successfully!']);
    }

    public function changeStatus($appointmentId, $status)
    {
        $updateStatus = $status == 0 ? 1 : 0;
        Appointment::find($appointmentId)->update(['status' => $updateStatus]);
    }

    public function getAppointmentsProperty()
    {
        $appointmentsQuery = Appointment::search($this->search);
    }

    public function updatedSelectAll($value)
    {
        if ($value) {
            $this->checked = Appointment::get()->pluck('id')->map(fn ($value) => (string) $value)->toArray();
        } else {
            $this->checked = [];
        }
    }

    public function updatedChecked()
    {
        if (count($this->checked) == Appointment::count()) {
            $this->selectAll = true;
        } else {
            $this->selectAll = false;
        }
    }

    public function SelectAllRecord()
    {
        $this->checked = Appointment::get()->pluck('id')->map(fn ($value) => (string) $value)->toArray();
    }

    public function deleteId($id)
    {
        $this->deleteId = $id;
    }

    public function confirmAppointmentsRemoval($appointmentId)
    {
        if (!empty($this->deleteId)) {
            $this->appointmentIdBeingRemoved = $appointmentId;
            $this->dispatchBrowserEvent('show-delete-modal');
        } else {
            $this->dispatchBrowserEvent('show-multi-delete-modal');
        }
    }

    public function delete()
    {
        if (!empty($this->deleteId)) {
            Appointment::find($this->deleteId)->delete();
        } else {
            Appointment::whereIn('id', $this->checked)->delete();
        }
        $this->selectAll = false;
        $this->checked = [];
        $this->dispatchBrowserEvent('hide-multi-delete-modal', ['message' => 'Selected appointments deleted successfully!']);
    }


    public function render()
    {
        $appointments = Appointment::all();
        return view('livewire.admin.appointment.appointments', [
            'appointments' => $appointments,
        ])
            ->extends('layouts.admin.app');
    }

    // public function index()
    // {
    //     return view('admin.appointment.index');
    // }
}
