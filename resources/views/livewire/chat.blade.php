                                ...
                                <li class="{{ $conversation->id === $selectedConversation->id ? 'bg-warning' : '' }}">
                                    <a href="#" wire:click.prevent="viewMessage({{ $conversation->id }})">
                                        <img class="contacts-list-img"
                                            src="{{ $conversation->sender_id === auth()->id() ? $conversation->receiver->avatar_url : $conversation->sender->avatar_url }}"
                                            alt="User Avatar">
                    ...
                    <div class="card-body">
                        ...
                        <div class="direct-chat-messages" id="conversation">
                            ...
                            @foreach ($selectedConversation->messages as $message)
                                <div
                                    class="direct-chat-msg {{ $message->user_id === auth()->id() ? 'right bg-info' : 'bg-danger' }}">
                    ...
                    <div class="card-footer">
                        <form wire:submit.prevent="sendMessage" action="#">
                            <div class="input-group">
                                <input wire:model.defer="body" type="text" name="message"
...
@else
    <div class="error-box d-flex flex-column min-vh-100 justify-content-center align-items-center">
        <h2><i class="fa fa-envelope"></i> You do not have any message yet!</h2>
        <p>Click the button below to select the users to chat with</p>
        <a href="{{ route('users.index') }}" class="btn btn-custom">Go to Users</a>
    </div>
